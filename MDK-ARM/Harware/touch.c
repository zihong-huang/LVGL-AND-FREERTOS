 ...
 * @brief 在触摸屏上绘制线条
 * 
 * 该函数会根据用户的触摸动作，在屏幕上绘制连续的线条。
 */
void TOUCH_DrawLine(uint16_t color)
{
    uint16_t lastX = 0, lastY = 0;   // 上一次触摸点的坐标
    uint16_t currentX, currentY;     // 当前触摸点的坐标
    ...
    while(1)
    {
        ...
        if(TOUCH_Scan() == 1)
        {
            currentX = TouchData.lcdx; // 获取当前触摸点的X坐标
            currentY = TouchData.lcdy; // 获取当前触摸点的Y坐标
            ...
            // 画线从上一个点到当前点
            LCD_DrawLine_Color(lastX, lastY, currentX, currentY, color);
